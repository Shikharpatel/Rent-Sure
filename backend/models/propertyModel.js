...
const Property = {
    ...
    create: async (ownerId, address, city, rentAmount, estimatedDeposit, buildingYear, inviteCode) => {
        const query = `
      INSERT INTO Properties (owner_id, address, city, rent_amount, estimated_deposit, building_year, invite_code)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *;
    `;
        const values = [ownerId, address, city, rentAmount, estimatedDeposit, buildingYear || null, inviteCode];
    ...
    // Update a property
    update: async (propertyId, address, city, rentAmount, estimatedDeposit, buildingYear) => {
        const query = `
      UPDATE Properties
      ...
      RETURNING *;
    `;
        const values = [address, city, rentAmount, estimatedDeposit, buildingYear || null, propertyId];
        const result = await db.query(query, values);
        return result.rows[0];
    },
